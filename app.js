let map, markers = [], latlngs = [], polyline = null, control = null;
let actionHistory = [];
...
function undoAction() {
  ...
  if (last.type === 'add') {
    ...
    if (actionHistory.length && actionHistory[actionHistory.length - 1].type === 'remove') {
      actionHistory.pop();
    }
...
function redrawRoute(order) {
  ...
  const waypoints = order.map(i => L.latLng(latlngs[i][0], latlngs[i][1]));
